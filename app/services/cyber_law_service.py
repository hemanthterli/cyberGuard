...
def _google_search_laws(
    ...
    Falls back to ([], search_query) on any error.
    """
    ...
    try:
        ...
        logger.warning("Google Search call failed during enrichment", exc_info=True)
        return [], search_query
    ...
    # Use search queries generated by the model (if available)
    if grounding and hasattr(grounding, "web_search_queries"):
        used_queries = list(grounding.web_search_queries or [])
        if used_queries:
    ...
    if grounding_chunks:
        ...
        structured.append(
            {
                "title": f"Google Search: {query} in {country_label}",
    ...
    logger.info("Google Search returned %d structured entries for query: %s", len(structured), search_query)
...
def _rerank_with_gemini(
    ...
    try:
        ...
        gemini_error_handler.raise_if_model_busy(exc)
        logger.warning("Gemini re-ranking call failed, using all laws", exc_info=True)
        return all_laws
